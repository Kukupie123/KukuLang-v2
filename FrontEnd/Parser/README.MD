# KukuSpeak Parser doc
## NOTE TO SELF : Stop Treating Parser like an interpreter. Let it only parse instructions, structures, functions and place them properly in Tree format.
```
Define Human with age(number), name(text).
Define Foo returning nothing with ~Funky comment~ num1(number).
Define Square returning integer with num1(number).

Set a to Square's output with num1(12). ~function call~

Set a to 12.
Set b to "Kuku".

Set c to Human.
Set d to Human with age as a, name as b.

Set c's age to 24.

Set d to (Square with num1 as 21)'s output.
~we can also access property of output by using "'s" similarly to how we do for structures~

Set f to d's age.
~We can also change access~
Set f to computer's mouse's button's name.
...
```
class SetScopeVariableStatement{
    string variableName;
...
set b to a's typer.
```
Statements have no return type
1. Create/update scope instance Stmt
2. Update nested instance
# Expressions
Expressions have return type
1. Binary Expressions (+,-,*,/,<=,>=,==,!=,&&,||)
2. instance expressions (CustomType)
